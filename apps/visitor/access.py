    ...
    def post(self,request):
        try:
            ...
            if (visitor_pass.valid_until>=timezone.now()) and (not visitor_pass.is_cancelled) and (not visitor_pass.visitor.is_blacklisted) and ( not visitor_pass.visitor.is_deleted):
                serilizer = VisitorSerializer(visitor_pass.visitor)
                data = serilizer.data
                data["detail"] = "Gate Opened"
                return Response(data,status=200) if(activate(adam_data.adam.ip,adam_data.adam.actuation_port)) else Response({"detail":"Something went wrong Adam device"},status=500)
